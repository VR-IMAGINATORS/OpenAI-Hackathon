...
export function publicOrigin(value: string): string {
  ...
  try {
    ...
    throw new Error('PUBLIC_GAME_URLはHTTPSのURLにしてください。');
  }
  if (
    url.protocol !== 'https:' ||
    url.username ||
    ...
    throw new Error('PUBLIC_GAME_URLは認証情報やパスのないHTTPS originにしてください。');
  return url.origin;
}
...
export function startTunnel(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(
        new Error(
          'トンネルの準備が時間内に完了しませんでした。回線とcloudflaredの設定を確認してください。',
        ),
      );
    }, 45_000);
    ...
    child.once('error', () => {
      clearTimeout(timer);
      reject(new Error('cloudflaredを起動できません。インストール後に再実行してください。'));
    });
    child.once('exit', () => {
      clearTimeout(timer);
      if (resolved) onExit();
        ...
        reject(
          new Error('トンネルが終了しました。cloudflaredの設定とネットワークを確認してください。'),
        );
    });
  });
}
