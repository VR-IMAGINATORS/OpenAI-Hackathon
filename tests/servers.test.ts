...
test('full local → required relay → mock path exposes no credentials or private scenario', async t => {
  ...
  const result = await post(browser.url + '/api/connection', { passphrase: 'local-demo-only' }, { origin: browser.url });
...
test('auth none still issues bounded authenticated sessions and honors expiry', async t => {
  let now = 1000;
  const relay = await listen(t, createRelayApp(relayConfig({ authMode: 'none', tokenTtlMs: 50, tokenMaxRequests: 1, now: () => now })));
...
test('global session count remains capped after token expiry', async t => {
  let now = 1000;
  const relay = await listen(t, createRelayApp(relayConfig({ authMode: 'none', maxSessions: 1, tokenTtlMs: 10, now: () => now })));
...
test('global request count applies across sessions', async t => {
  ...
  assert.equal((await post(relay.url + '/v1/diagnostics', {}, { authorization: 'Bearer ' + one })).status, 200);
  assert.equal((await post(relay.url + '/v1/diagnostics', {}, { authorization: 'Bearer ' + two })).status, 429);
  assert.equal((await post(relay.url + '/v1/sessions')).status, 429);
});
test('auth attempts consume the fixed window and recover when the window elapses', async t => {
  let now = 100;
  const relay = await listen(t, createRelayApp(relayConfig({ authMaxAttempts: 2, authWindowMs: 20, now: () => now })));
  assert.equal((await post(relay.url + '/v1/sessions', { passphrase: 'bad' })).status, 401);
  assert.equal((await post(relay.url + '/v1/sessions', { passphrase: 'bad' })).status, 401);
  assert.equal((await post(relay.url + '/v1/sessions', { passphrase: 'local-demo-only' })).status, 429);
  now = 120;
  assert.equal((await post(relay.url + '/v1/sessions', { passphrase: 'local-demo-only' })).status, 200);
});
test('Host and Origin are independently checked, with explicit dev proxy and CLI allowed', async t => {
  ...
  assert.equal((await fetch(browser.url + '/api/bootstrap', { headers: { host: 'evil.invalid', origin: 'http://evil.invalid' } })).status, 403);
  assert.equal((await fetch(browser.url + '/api/bootstrap', { headers: { origin: 'https://evil.invalid' } })).status, 403);
  assert.equal((await fetch(browser.url + '/api/bootstrap', { headers: { origin: 'null' } })).status, 403);
  assert.equal((await fetch(browser.url + '/api/bootstrap', { headers: { host: 'localhost:5173', origin: 'http://localhost:5173' } })).status, 200);
...
test('invalid, oversized and arbitrary forwarding requests are rejected without reflecting payloads', async t => {
  ...
  for (const url of [relay.url + '/v1/sessions', browser.url + '/api/connection']) {
    const malformed = await fetch(url, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{"secret":' });
    assert.equal(malformed.status, 400);
    assert.doesNotMatch(await malformed.text(), /secret/);
    assert.equal((await post(url, { passphrase: 'x'.repeat(40_000) })).status, 413);
    assert.equal((await post(url, { passphrase: 'local-demo-only', url: 'https://arbitrary.invalid' })).status, 400);
  }
});
test('bootstrap reports unavailable and connection fails if relay is stopped', async t => {
  ...
  const bootstrap = await fetch(browser.url + '/api/bootstrap');
  assert.deepEqual((await bootstrap.json() as { relay: unknown }).relay, { reachable: false, authMode: null, mode: null });
...
test('upstream errors and valid diagnostic messages never leak into browser output', async t => {
  ...
  const failure = await listen(t, (_req, res) => { res.statusCode = 500; res.end(upstreamSecret); });
  ...
  const success = await listen(t, (req, res) => {
    ...
    res.end(JSON.stringify(req.url === '/v1/sessions'
...
test('upstream redirects are rejected without contacting the redirect target', async t => {
  let redirected = 0;
  const target = await listen(t, (_req, res) => { redirected++; res.end('{}'); });
  const relay = await listen(t, (_req, res) => { res.writeHead(302, { location: target.url }); res.end(); });
  const browser = await local(t, relay.url);
  assert.equal((await post(browser.url + '/api/connection')).status, 502);
...
test('one connection deadline includes both operations and stalled response body reads', async t => {
  ...
  const streaming = await listen(t, (_req, res) => { res.writeHead(200, { 'content-type': 'application/json' }); res.write('{'); });
  ...
  assert.equal((await bootstrap.json() as { relay: { reachable: boolean } }).relay.reachable, false);
});
...
test('invalid relay JSON and invalid response shapes are rejected without exposure', async t => {
  ...
  const relay = await listen(t, (req, res) => {
    res.setHeader('content-type', 'application/json');
    res.end(req.url === '/health' ? JSON.stringify({ service: 'relay', mode: 'real', authMode: 'none', secret }) : '{' + secret);
  });
  const browser = await local(t, relay.url);
  const bootstrap = await fetch(browser.url + '/api/bootstrap');
...
test('auth-none relay can be diagnosed through the local server without passphrase', async t => {
  ...
  const browser = await local(t, relay.url);
  const bootstrap = await fetch(browser.url + '/api/bootstrap');
  assert.equal((await bootstrap.json() as { relay: { authMode: string } }).relay.authMode, 'none');
