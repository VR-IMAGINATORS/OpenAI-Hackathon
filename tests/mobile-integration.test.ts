...
test('mobile owner plays three obstacles through local and authenticated relay with fake OpenAI', async t => {
  ...
  let recognitionCalls = 0, judgmentCalls = 0, hangups = 0, liveCreates = 0;
  const relayConfig: RelayConfig = {
    ...loadRelayConfig({ FOUNDATION_DEMO: '1' }), mode: 'live', passphrase, apiKey,
    authGlobalMaxAttempts: 100,
    live: { liveModels: ['gpt-live-1'], responseModels: ['integration-vision'], tokenLiveAttempts: 3,
      tokenResponseAttempts: 40, globalLiveAttempts: 3, globalResponseAttempts: 40,
      globalLiveConcurrent: 1, globalResponseConcurrent: 1, durationMs: 600000,
      heartbeatMs: 30000, outputTokens: 1000 },
  };
  const relay = createRelayApp(relayConfig, {
    async createLiveSession(body) {
      liveCreates++;
      const value = body as { session: { model: string; instructions: string }; transport: { type: string; sdp: string } };
      ...
      return { session: { id: 'live_integration' }, transport: { type: 'webrtc', sdp: 'integration-answer' } };
    },
    async createResponse(body) {
      const value = body as { model: string; store: boolean; input: { content: { type: string; text?: string; image_url?: string }[] }[] };
      ...
      const context = JSON.parse(parts.find(p => p.type === 'input_text')!.text!) as {
        photos: { id: string }[]; transcript: string; proposal?: unknown;
        inventory: { id: string }[];
      };
      const picture = parts.find(p => p.type === 'input_image');
      assert.match(picture!.image_url!, /^data:image\/jpeg;base64,/);
      const metadata = await sharp(Buffer.from(picture!.image_url!.split(',')[1]!, 'base64')).metadata();
      ...
      if (context.proposal) {
        ...
        result = { success: true, narrative: '工夫した道具で障害を突破した。', situation: '先へ進めるようになった。', inventoryChanges: [] };
      } else {
        recognitionCalls++;
        result = { items: [{ photoId: context.photos[0]!.id, inventoryId: null, name: '丈夫な道具' }],
          usage: context.transcript ? '道具をてことして使う' : '', summary: context.transcript ? '道具をてことして使う案' : '用途を相談してください' };
      }
      return { output: [{ type: 'message', content: [{ type: 'output_text', text: JSON.stringify(result) }] }] };
    },
    async hangup(id) { assert.equal(id, 'live_integration'); hangups++; },
  });
  ...
  t.after(async () => {
    ...
    localServer.closeAllConnections(); relayServer.closeAllConnections();
    await Promise.all([new Promise<void>(resolve => localServer.close(() => resolve())), new Promise<void>(resolve => relayServer.close(() => resolve()))]);
  });
  let owner = '';
  async function request(path: string, body?: unknown, method = 'POST', authenticated = true) {
    const response = await fetch(origin + path, { method, headers: { Origin: origin, 'Content-Type': 'application/json', ...(authenticated && owner ? { Cookie: owner } : {}) }, body: body === undefined ? undefined : JSON.stringify(body) });
  ...
  assert.equal((await request('/api/play/heartbeat', { generation, voiceState: 'connected' })).response.status, 200);
  assert.equal((await request('/api/play/start', {})).data.status, 'playing');
  const photo = (await sharp({ create: { width: 32, height: 32, channels: 3, background: '#aaaaaa' } }).png().toBuffer()).toString('base64');
  ...
  for (let obstacle = 0; obstacle < 3; obstacle++) {
    ...
    const event = await request('/api/play/events', { generation, event: { type: 'session.input_transcript.delta', event_id: randomUUID(), delta: 'この道具をてことして使ってください', start_ms: obstacle * 1000, end_ms: obstacle * 1000 + 800 } });
    assert.equal(event.response.status, 200, event.raw);
    assert.equal(event.data.state.proposal, null);
    const recognized = await request('/api/play/events', { generation, event: { type: 'session.delegation.created', event_id: randomUUID(), offset_ms: obstacle * 1000 + 800, delegation: { id: 'delegate_' + obstacle, type: 'delegation', target: 'client' } } });
