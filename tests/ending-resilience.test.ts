...
async function setup(t: TestContext, options: {
  ready?: boolean;
  faults?: boolean;
  fatal?: 'direction-auth' | 'direction-refusal' | 'frame-auth' | 'inspection-refusal' | 'image-budget';
  waitMs?: number;
} = {}) {
  let now = 0;
  const snapshot = new ScenarioCatalog({
    scenarioPath: 'scenarios/mobile-playtest.json', coreConfigPath: 'config/game-core.json',
  }).current('en');
  const facts = {
    obstacleId: snapshot.scenarioV2.obstacles[0].id,
    values: Object.fromEntries(snapshot.scenarioV2.core.facts.map((fact) => [fact.key, fact.initial])),
  };
  const playId = randomUUID();
  const packet: EndingPacket = {
    playId, snapshot, scenario: localizeScenario(snapshot.scenarioV2, 'en'), locale: 'en',
    outcome: 'bad', endReason: 'time_limit', clearedIds: [], remainingObstacles: [],
    facts, inventory: [{ id: 'rope', name: 'rope', description: 'frayed', status: 'damaged' }],
    actions: [{
      actionId: 'attempt', order: 1, obstacleId: facts.obstacleId, usage: 'Pull with the rope',
      items: [{ id: 'rope', name: 'rope', beforeStatus: 'available', afterStatus: 'damaged' }],
      beforeVersion: 0, afterVersion: 1, beforeFacts: facts, afterFacts: facts,
      success: false, cleared: false, narrative: 'The rope frayed; the obstacle remains locked.',
    }],
    evidence: { records: [], truncated: false }, endedAt: 0, gameVersion: 1,
    finalMessageId: null, recentActionScenes: [],
  };
  const jpeg = await sharp({ create: { width: 1024, height: 1024, channels: 3, background: '#456' } })
    .jpeg().toBuffer();
  const counts = { text: 0, direction: 0, start: 0, end: 0, startChecks: 0, endChecks: 0, submit: 0, result: 0, download: 0 };
  ...
  const ai = new AiService(config, {
    async createLiveSession() { throw new Error('unused'); },
    async hangup() {},
    async createResponse(body) {
      ...
      if (name === 'ending_text') {
        ...
        return response({ title: 'The locked room', story: 'The rope frayed and the room remained locked.', evaluation: 'The attempt failed.', tag: null, usedEvidenceIds: [] });
      }
      if (name === 'ending_design') {
        counts.direction++;
        if (options.fatal === 'direction-auth') throw new UpstreamError(502, 401);
        if (options.fatal === 'direction-refusal') return { output: [{ content: [{ type: 'refusal' }] }] };
      ...
      checked.push({ slot: context.slot, image: b.input[0].content[1].image_url, target: context.target });
      const count = context.slot === 'start' ? ++counts.startChecks : ++counts.endChecks;
      if (options.fatal === 'inspection-refusal') return { output: [{ content: [{ type: 'refusal' }] }] };
      if (options.faults) {
        if (context.slot === 'start' && count === 1) throw new UpstreamError(502, 503);
        if (context.slot === 'end' && count === 1) return response({ verdict: 'reject', problems: ['Restore the unresolved restraint.'] });
    ...
    async createImageEdit(body) {
      ...
      if ((options.faults || options.fatal === 'image-budget') && slot === 'start' && counts.start === 1)
  ...
  const assetId = await results.putAsset(playId, { kind: 'scene', mime: 'image/jpeg', bytes: jpeg });
  const messageId = randomUUID();
  const releaseReference = (status: 'ready' | 'failed' = 'ready') => results.updateMessage(playId, messageId, {
    imageSlot: { status, assetId: status === 'ready' ? assetId : null, errorCode: null, deadline: new Date(1_030_000).toISOString() },
  });
  results.appendMessage(playId, {
    id: messageId, side: 'assistant', kind: 'result', text: 'Confirmed scene',
    imageSlot: { status: 'checking', assetId: null, errorCode: null, deadline: new Date(1_030_000).toISOString() },
  });
  results.bindScene(playId, messageId, 0);
  if (options.ready) releaseReference();
  const recoveries: string[] = [];
  const jobs = new EndingJobs(ai, {
    enabled: true, apiKey: 'fake', globalAttempts: 1, timeoutMs: 180_000, concurrent: 1,
  }, results, {
    now: () => now, graceMs: 0, retryDelayMs: 1, referenceWaitMs: options.waitMs ?? 100,
    ...
    fal: {
      ...
      async status(received) { assert.deepEqual(received, handle); return 'COMPLETED'; },
      async result() {
        if (++counts.result === 1 && options.faults) throw new FalTransportError();
        return { videoUrl: 'https://fal.media/test.mp4' };
      },
      async downloadVideo() {
        if (++counts.download === 1 && options.faults) throw new FalTransportError('FAL_DOWNLOAD_FAILED');
        return syntheticEndingMp4();
      },
      async cancel() { return { stopConfirmed: true }; },
    },
  });
  t.after(async () => { await jobs.drain(); jobs.dispose(); await ai.shutdown(); });
  const start = () => { jobs.enqueue(packet, () => packet); results.end(playId, { status: 'lost' }); };
  return { ai, jobs, results, packet, start, releaseReference, counts, recoveries, checked,
    view: () => results.ending('owner', playId), setNow: (value: number) => { now = value; } };
}
...
test('late scene, text failure, direction timeout, image failure, inspection failure and fal read failures still complete one video', async (t) => {
  ...
  assert.deepEqual(f.counts, { text: 1, direction: 1, start: 2, end: 2, startChecks: 2, endChecks: 3, submit: 1, result: 2, download: 2 });
  assert.equal(f.checked[0].image, f.checked[1].image, 'inspection retries reuse the same generated image');
...
for (const fatal of ['direction-auth', 'direction-refusal', 'frame-auth', 'inspection-refusal', 'image-budget'] as const)
...
for (const reason of ['cancel', 'expire', 'wait-timeout', 'all-images-failed'] as const)
  test(`reference wait stops on ${reason} and a late image cannot restart the ending`, async (t) => {
    ...
    if (reason === 'expire') { f.setNow(180_000); f.jobs.tick(); }
...
test('aftermath fallback uses the supplied outcome, without inventing action/evidence IDs', async (t) => {
  ...
  for (const outcome of ['happy', 'normal', 'bad'] as const) {
    ...
    assert(design.videoPrompt.includes(outcome === 'happy' ? 'confirmed completed escape' : 'has not escaped'));
  }
});
