...
`judge-packet` first validates recognition, then returns only the current gimmick's physical contract, the intent, recognition, and selected inventory. It contains no later obstacle or ending. Use that packet for the Sol low adjudication prompt. Escalate the same pending action to Sol high only when low cannot produce a valid, physically decisive judgment.
...
```powershell
python -X utf8 $Game attach-image --session "runs/call-to-past/<session-id>" --event-id turn-01 --file "C:/absolute/path/result.png" --media-kind generated --provenance "OpenAI Image API; gpt-image-2.5-flare; quality=low; 1536x1024"
python -X utf8 $Game context --session "runs/call-to-past/<session-id>"
```
...
python -X utf8 $Game attach-ending --session "runs/call-to-past/<session-id>" --file "C:/absolute/path/ending.png" --media-kind generated --provenance "OpenAI Image API; gpt-image-2.5-flare; quality=low; 1536x1024"
