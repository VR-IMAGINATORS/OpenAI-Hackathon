...
3. `title`、`story`、`evaluation`の3つの文字列を持つstory.jsonを保存し、`attach-story --story-json ...`で履歴へ登録する。次に[画像手順](media.md)の承認済みAPI条件を使い、直前の結果画像と同じ人物/衣装/空間で開始・終了フレームを`gpt-image-2.5-flare`から生成する。標準は`quality=low`、`1536x1024`、各1枚。両方のサイズ・縦横比を揃え、事前に閲覧。顔は背面・遮蔽・口元以下で全編を計画する。終了フレームを`attach-ending`で保存し、開始・終了の実ファイルはH3のprepareへ渡す。
...
6. submitは一度だけ。request IDを保存してstatus/resultから回収する。不明時に新しい枠を作って再送しない。h3-videoのQCと視聴により尺、再生、音、人物/空間、顔の露出を確認。抽出12フレームの確認だけで全編を確認済みとは書かない。回収した動画を`attach-video`で登録する。liveでは`--h3-run`と`--receipt`も渡し、承認・request ID・素材・動画の照合を通す。具体的な引数はcli.mdとh3-cli.mdを読む。
