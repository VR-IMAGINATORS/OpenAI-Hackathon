...
export class PlayAccess {
  ...
  currentInvite() {
    ...
    return {
      url: this.origin + '/#invite=' + this.invitation.value,
  ...
  async expire() {
    if (this.owner && this.owner.expiresAt <= this.now()) await this.clear();
  }
  claim: RequestHandler = async (req, res) => {
    await this.expire();
    if (this.now() - this.windowAt >= 60_000) {
      this.windowAt = this.now();
      this.attempts = 0;
    }
    if (++this.attempts > 30) {
      errorResponse(res, 429, 'INVITE_LIMIT', '参加試行が多すぎます。少し待ってください。');
      return;
    }
    if (this.owner || this.clearing || this.resetFailed) {
      errorResponse(
        res,
        409,
        'PLAY_OCCUPIED',
        'すでに参加中です。元のブラウザで開くか、PCからやり直してください。',
      );
      return;
    }
    if (
      ...
      !equal(req.body.invite, this.invitation?.digest) ||
      !this.invitation ||
      ...
      errorResponse(
        ...
        401,
        'INVITE_INVALID',
        '招待の期限が切れたか、使用済みです。PCからQRを再発行してください。',
      );
      return;
    }
    const value = token();
    ...
    this.invitation = undefined;
    res.cookie('play_owner', value, {
      httpOnly: true,
      sameSite: 'strict',
      secure: this.origin?.startsWith('https:') ?? false,
      maxAge: 900_000,
      path: '/api/play',
    });
    res.json({ ok: true });
  ...
  authorize: RequestHandler = async (req, res, next) => {
    ...
    if (this.clearing || !this.owner || !equal(cookie(req, 'play_owner'), this.owner.digest)) {
      errorResponse(
        ...
        401,
        'OWNER_REQUIRED',
        'PCの招待QRから参加してください。期限切れの場合はQRを再発行してください。',
      );
      return;
    }
    next();
...
export class AdminAccess {
  ...
  authorize: RequestHandler = (req, res, next) => {
    if (this.now() >= this.expiresAt || !equal(cookie(req, 'play_admin'), this.digest)) {
      errorResponse(res, 401, 'ADMIN_REQUIRED', '起動時に表示された管理リンクから開いてください。');
      return;
    }
    next();
  };
...
export function originGuard(hosts: Set<string>, origins: Set<string>): RequestHandler {
  return (req, res, next) => {
    ...
    if (
      !hosts.has((req.get('host') ?? '').toLowerCase()) ||
      (origin !== undefined && !origins.has(origin)) ||
      (!['GET', 'HEAD', 'OPTIONS'].includes(req.method) && !origin)
    ) {
