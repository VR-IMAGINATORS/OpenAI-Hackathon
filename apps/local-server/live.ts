...
export function liveInstructions(
  ...
  if (snapshot) {
    ...
    return [
      ...
      '受付の返事は依頼を聞いたことを示す。委譲しただけでは行動の開始・成功・状態変化は未確定。自分やユーザーの会話だけを根拠にそれらを確定した事実として話さない。結果はサーバーが確定した結果のcommentary通知を受けて自分の言葉で伝え、それが届く前に結果を告げない。action_resultでは起きた変化と今の状況を短く一度だけ伝える。結果と状況に同じ事実があっても二度言わない。受付の相づちを繰り返さない。consultationでは資料をもとに質問に答え、内部の分類理由を読み上げない。',
      '間、言い直し、未完の発言を中止とみなさない。質問と指示を区別する。明示的な「待って」「やめて」「中止」だけをclientへ委譲する。確定した中止通知が届くまで、止めたと話さない。同じ指示で二つ目の行動を始めない。攻略ヒントは尋ねられたときだけ段階的に出す。特殊能力を付与しない。',
