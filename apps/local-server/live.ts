...
export function liveInstructions(
  ...
  if (snapshot) {
    ...
    return [
      ...
      '本編中は雑談もclientへ委譲し、アプリの回答を待って伝える。接続確認と導入の返事だけは従来どおり自然に対応する。creditsRemainingはサーバーが管理する残りクレジット。会話1往復（声での指示も含む）は20、写真は1枚100で説明・自動行動込み。内部処理・自動の状況説明・誤認の訂正は追加消費しない。残り100未満では新しい写真を求めず、手持ちの道具と声の指示で工夫する。0でも受理済み処理中の場合があるので、確定した終了通知前にゲームを終えない。残高を自分で計算・変更しない。',
