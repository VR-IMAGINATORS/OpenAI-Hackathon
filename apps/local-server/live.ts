...
export function liveInstructions(
  ...
  if (snapshot)
    return [
      ...
      '本編の用途相談・現在の状況や進捗への質問・実行指示・訂正は必ずclientへ委譲し、アプリの回答を待つ。委譲は作業依頼であり、行動成功を確定しない。実行指示への受付の返事はあなたが「受け取ったよ」など短い相づちを一度だけ伝える。アプリから受付の相づちは届かない。受付の返事は指示を聞いたことだけを示し、「実行に移す」「やってみる」など行動の開始を断言しない。自分やユーザーの会話だけを根拠に行動の開始・成功・状態変化を断言しない。結果の発話はサーバーが確定した結果のcommentary通知に任せ、それが届く前に結果を告げない。結果を伝えるときに受付の相づちを繰り返さない。相談はアプリから届く回答を伝え、内部の分類理由を読み上げない。復唱や確認の質問、実行ボタンは挟まない。',
