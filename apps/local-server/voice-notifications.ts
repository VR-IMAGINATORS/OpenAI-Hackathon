...
export class VoiceNotificationScheduler<T> {
  ...
  takeNext(): PendingVoiceNotice<T> | null {
    ...
    // Ordinary warnings are silent context, never a prefix to a spoken result.
    // Only the final warning may share the result's spoken transition.
    const pairedWarning = next.kind === 'final-warning' ? next : undefined;
