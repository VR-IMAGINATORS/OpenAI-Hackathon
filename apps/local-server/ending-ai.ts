...
Compare THREE concise scene ideas: two recent actions connected, one recent action, and aftermath. Choose a readable 15-second sequence within allowedModes, using at most the supplied recent action IDs, in chronological order. For actions include physical contact/support, result and bodily reaction. If before-action visual evidence is missing, set mode=aftermath and depict confirmed aftermath only. For no actions use initial constraints and time pressure without a fictitious attempt.
The film should explain which player-supplied tools enabled progress and how they worked. When preferredSequenceActionIds is nonempty, prefer mode=actions with BOTH of those IDs in the supplied chronological order: first method and its confirmed result, a motivated cut to the next method and its confirmed result, then reaction and the final outcome. Give each action a distinct visible beat, especially when they concern different obstacles; changing camera angles on the same interaction does not show two actions. Use a short cut to omit uneventful travel or tool handover before dropping an action. Keep failures and partial results exactly as committed. If the sequence still cannot be read in 15 seconds, explain the specific timing or continuity obstacle and the omitted action in selectionReason, then choose one action. preferredActionIds ranks successful tool actions with before-action references for that single-action fallback (latest clear first). When present, prefer mode=actions and include a preferred action. Explain any choice of aftermath instead in selectionReason. Stage each identifiable tool, its contact point, the confirmed usage motion and the actual result before escape/reaction. Merely holding a tool or showing an already open door does not explain the method. Keep the established operator; never invent hands or a body for a bodiless AI, replace an unusual method with a conventional solution, or grant a new ability.
For two actions, plan when each tool appears, where it is held or supported, and where it stays after use. A tool first introduced by the second action must not appear in the opening. Preserve damage, consumption, screen direction and the first result across the cut; never morph one tool or obstacle into another. Each selected action is a single depiction of an already committed event, not a new attempt.
...
Write videoPrompt as one English prompt string with explicit numbered shots: [Shot 1] for the opening, then [Shot 2] At 00:SS.mmm, the camera cuts to ... and so on, with strictly increasing cut times inside 15 seconds. With two selected actions, use a distinct shot for each action and a final outcome/reaction shot. For example, allow roughly 0-4 seconds for the first cause/result, 4-8.5 for the next, and 8.5-12 for the outcome/reaction; adjust to the actual motion rather than forcing equal durations. With one action, show its cause/result then cut to the outcome/reaction. Aftermath may stay in one shot when appropriate. Explicitly request these edits: first-and-last-frame interpolation alone tends to produce a single shot. Align the supplied start image with the opening shot and the supplied end image with the final shot at 15 seconds. For each shot give camera height/distance/direction, subject motion distinct from camera movement, the visible change and synchronized physical sound; motivate each cut with new action or outcome information. Keep this staging in videoPrompt, not only in candidates or selectionReason. Spend the existing response budget on these concrete shots, keeping candidates, rationale and frame prompts concise. No speech, narration, singing or music; only ambience and physical sound.
...
Write all image/video prompts in English. The film and established short story must agree on the confirmed outcome; the tag may reflect an earlier action outside the film's recent action selection.`;
  ...
  const preferredActionIds = canReplayActions
    ...
    : [];
  // Prefer the connected pair without another AI call, image, or reference wait.
  // Only its opening needs a before-action reference, as in the replay validator.
  const preferredSequenceActionIds =
    canReplayActions &&
    recent.length === 2 &&
    preferredActionIds.length > 0 &&
    availableBefore.some((reference) => reference.gameVersion === recent[0].beforeVersion)
