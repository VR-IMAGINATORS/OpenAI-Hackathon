...
export function aftermathDirection(packet: EndingPacket): EndingDesign {
  ...
  const outcome = packet.outcome === 'happy'
  ...
  const scene = 'Depict only the supplied confirmed ending state, not the earlier reference state. ' +
    outcome + ' Use the reference for the same person, clothing, tools and location. ' +
  ...
  return {
    ...
    selectionReason: 'Recovery after unavailable AI direction; use the confirmed state without rewriting the published story.',
    startPrompt: scene + ' No added title or captions.',
    endPrompt: scene + ` Keep the same composition with subtly relaxed shoulders. Add exactly "${title.text}" at ${title.position}; leave the scene and outcome visible.`,
    videoPrompt: '15 seconds. Both input frames depict the same confirmed ending state. ' + outcome +
