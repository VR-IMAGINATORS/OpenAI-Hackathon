...
export async function classifyCoreIntent(options: {
  ...
  const instructions = [
    ...
    'creditsRemaining is the server-owned balance: a conversation exchange including a voice action costs 30, a new photo costs 150 per image including its automatic action. Existing tools need no new photo. At less than 150, use existing tools or conversation when at least 30 credits remain. The last paid operation may still be processing below 30. Only a confirmed terminal status ends play; never invent balances or extra charges.',
