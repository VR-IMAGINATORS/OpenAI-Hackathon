...
export async function playRequest<T>(path: string, body?: unknown, method = 'POST'): Promise<T> {
  ...
  try {
    ...
    if (!response.ok) throw new PlayApiError(value?.error?.message || '通信を完了できませんでした。', response.status);
    return value as T;
  } catch (error) {
    if (error instanceof PlayApiError) throw error;
    throw new PlayApiError(controller.signal.aborted ? '通信がタイムアウトしました。状態を確認して再試行してください。' : '通信できません。ネットワークとPCの起動状態を確認してください。', 0);
