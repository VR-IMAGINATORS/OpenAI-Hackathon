...
export default function JoinScreen({
  ...
  async function join() {
    ...
    try {
      try {
        ...
        if (!(error instanceof PlayApiError) || error.status !== 401) throw error;
      }
      if (invite.current) {
      ...
      setError(
        error instanceof PlayApiError && error.status === 401
  ...
  if (state)
    return (
      <PlayScreen initialState={state} needsPassphrase={bootstrap.relay.authMode !== 'none'} />
    );
  return (
