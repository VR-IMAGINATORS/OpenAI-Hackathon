...
export interface LiveLimits {
  ...
  tokenLiveAttempts: number;
  tokenResponseAttempts: number;
  globalLiveAttempts: number;
  globalResponseAttempts: number;
  globalLiveConcurrent: number;
  globalResponseConcurrent: number;
  durationMs: number;
  heartbeatMs: number;
  outputTokens: number;
}
...
export function loadRelayConfig(
  ...
  if (authMode !== 'required' && authMode !== 'none')
    throw new Error('RELAY_AUTH_MODE must be required or none');
  ...
  if (mode === 'live') {
    ...
    for (const name of [
      'RELAY_GLOBAL_LIVE_ATTEMPTS',
      'RELAY_GLOBAL_RESPONSE_ATTEMPTS',
      'RELAY_GLOBAL_LIVE_CONCURRENT',
      'RELAY_GLOBAL_RESPONSE_CONCURRENT',
      'RELAY_AUTH_GLOBAL_MAX_ATTEMPTS',
    ])
    ...
    const models = (name: string) => {
      const entries = values[name]
        ...
        .map((v) => v.trim())
        .filter(Boolean);
      if (!entries?.length || entries.some((v) => !/^[-a-zA-Z0-9.]+$/.test(v)))
        throw new Error(name + ' required');
      return entries;
    };
    live = {
      liveModels: models('RELAY_LIVE_MODELS'),
      responseModels: models('RELAY_RESPONSE_MODELS'),
      tokenLiveAttempts: positiveInteger(values, 'RELAY_TOKEN_LIVE_ATTEMPTS', 3, 100),
      tokenResponseAttempts: positiveInteger(values, 'RELAY_TOKEN_RESPONSE_ATTEMPTS', 40, 1000),
      globalLiveAttempts: positiveInteger(values, 'RELAY_GLOBAL_LIVE_ATTEMPTS', 1),
      globalResponseAttempts: positiveInteger(values, 'RELAY_GLOBAL_RESPONSE_ATTEMPTS', 1),
      globalLiveConcurrent: positiveInteger(values, 'RELAY_GLOBAL_LIVE_CONCURRENT', 1, 100),
      globalResponseConcurrent: positiveInteger(values, 'RELAY_GLOBAL_RESPONSE_CONCURRENT', 1, 100),
      durationMs: positiveInteger(values, 'RELAY_LIVE_DURATION_SECONDS', 600, 3600) * 1000,
      heartbeatMs: 30000,
      outputTokens: positiveInteger(values, 'RELAY_MAX_OUTPUT_TOKENS', 1000, 1000),
    };
  }
  return {
    mode,
    ...
    passphrase,
    tokenTtlMs:
      positiveInteger(values, 'RELAY_TOKEN_TTL_SECONDS', mode === 'live' ? 900 : 300, 86400) * 1000,
