...
[Story.md](../Story.md)のエンディング・映像化を読み、本文の文体と長さ、映像対応表、画像用と動画用のプロンプトを整える。本文は動画前に提示し、種別・タイトル・得点・評価は動画後に表示する。

1. 日本語100〜200文字程度の小説調の本文を作り、動作・感覚・成否・余韻を15秒で伝わるよう描く。0〜5秒は直後の状況から動き出し、5〜10秒は人物の移動や大きな動作、10〜15秒は変化した場所・状況と未来と過去のつながりを示す余韻。初期伏線に結び付く意味を一部返し、監禁理由や黒幕の輪郭は根拠がある範囲でのみ補完する。実際の行動、既に提示した事実、解除数と矛盾させない。
2. happyは未来の自分が脱出し着信が静かに切れる。normalは危機を部分的に回避して助かる可能性を残す。救援灯や人影は、提示済みの履歴と確定本文にある場合だけ使う。badは脱出できず、既に残した痕跡や通信の余韻を描く。新たな道具使用や再接続は追加しない。流血・死の断定を必須にしない。
3. `title`、`story`、`evaluation`の3つの文字列を持つstory.json、本文と3区間の映像対応表、開始/終了画像用プロンプト、動画用プロンプトを画像生成より前に保存する。成否・文字数・動作の整合を確認後、`attach-story --story-json ...`で登録し、`story`本文だけを会話で提示する。登録後は異なる本文へ差し替えられない。次に直前の結果画像と同じ人物/衣装/空間で開始・終了フレームをnative imagegenから生成する。両方のサイズ・縦横比を揃え、事前に閲覧。顔は背面・遮蔽・口元以下で全編を計画する。終了フレームを`attach-ending`で保存し、開始・終了の実ファイルはH3のprepareへ渡す。
...
6. submitは一度だけ。request IDを保存してstatus/resultから回収する。同梱QCと視聴により尺、再生、音、人物/空間、顔の露出に加え、本文・映像対応表の出来事、順序、道具、成否との一致を確認する。不一致を本文の書換えで隠さない。不明時に新しい枠を作って再送しない。抽出12フレームの確認だけで全編を確認済みとは書かない。回収した動画を`attach-video`で登録する。liveでは`--h3-run`と`--receipt`も渡し、承認・request ID・素材・動画の照合を通す。具体的な引数はcli.mdとh3-cli.mdを読む。
...
| normal | 拘束が残る場合は位置を変えず、可能な上体動作、カメラ、既存光で差を作る。窓への移動や合図は身体状態と確定本文が許す場合だけ。救援灯や人影は提示済みの履歴と本文にある場合だけ使う | 部分的回避・助かる可能性。未解除の出口から脱出させない |
| bad | 拘束範囲内の上体動作、影、送信灯、カメラ移動で差を作る。後退や道具で残した痕跡は確定履歴にある場合だけ描き、新しい道具使用を足さない | 脱出失敗。残る拘束と移動範囲を守り、追加の攻略行動を与えない |
