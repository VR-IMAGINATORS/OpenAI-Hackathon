...
After three clears or four committed actions, follow ending.md: generate/reuse a start frame and a title-bearing final frame from the last game image and committed history, without a storyboard. Prepare and recover H3 using both frames as described in h3-cli.md. Register the exact final frame before attaching the video:
...
python -X utf8 $Game attach-ending --session "runs/call-to-past/<session-id>" --file "C:/absolute/path/ending-end.png" --media-kind generated --provenance "Final game image and committed history; title included"
...
For live attachment, free-text provenance is insufficient. The core requires the receipt saved by `media.py result` and binds it to the H3 run's manifest, approval, saved request ID, input snapshots, receipt-adjacent video, current source video, and, only for an approved two-image run, the registered game ending-image hash. The standard two-image run requires the same final image to be registered with attach-ending. Previously approved start-only runs retain explicit null end_image and end_image_sha256 and need no attach-ending call. SHA casing is normalized. It also runs `ffprobe` and requires 15 seconds ±0.25, a 768-pixel frame dimension, and at least one audio stream. Motion, face framing, continuity, and overall visual quality still require human review. Rehearsal media remains separate and must not supply live H3 evidence.
