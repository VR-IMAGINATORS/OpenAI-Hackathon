...
2026-09-13。設計改訂案。映画演出の試作動画は生成済み。本番制作フローの実装と、今回追加したH3内タイトル生成の検証は未実施。

**プレイヤーの工夫が人物の運命を左右する、映画のクライマックスを作る。** ゲーム履歴を短縮して並べるのではなく、履歴によって生まれた状況から、一つの切迫した場面を脚色する。

本資料を今回の設計検討の入口とする。以前の品質改善案とcinema-v2は参考候補に戻す。cinema-v2の「矢印→カード枠」や2ショット構成も採用確定ではない。既存の送信用manifestは保留し、新しい生成の承認に流用しない。
...
flowchart TD
  ...
  H --> I[再生]
  F -. 開発時に必要なら .-> J[絵コンテ・配置検証]
```

本番では画像の絵コンテ・配置図・終了画像を必須工程にしない。開始画像は外見と最初の状況を合わせるもの。選んだ脚本が既存画像に合わなければ、画像を先に固定して脚本を歪めず、開始画像の構図を見直す。

終了画像なしを今回の標準案とするが、効果は未検証。将来は明確な終端構図が必要な作品で任意に使える余地を残す。利用APIの入力条件は送信時に確認する。
...
タイトルについては正確な綴り、余計な文字の有無、結末より後の登場、読める静止時間、証拠の遮蔽、VFXによるゲーム内状態の誤読を確認する。文字と動画の全面禁止指示が混在しないことも送信前に確認する。旧`staging.py`の文字禁止は未改修なので、今回の文章プロンプトへそのまま取り込まない。
...
今回のタイトル方針改訂では文書と制作ガイダンスを更新。文字入り動画の新規生成、Web機能追加、通常プレイ登録契約の変更は行っていない。過去の試作動画・後付け文字版は比較記録として保持する。
