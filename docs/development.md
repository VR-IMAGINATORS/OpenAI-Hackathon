...
Prettierを開発依存として固定しています。リポジトリのルートで `npm run format` を実行すると、apps・packages・tools・testsのソースとscenariosのJSONを整形できます。`npm run format:check` はファイルを変更せず、書式だけ確認します。

設定は `.prettierrc.json`（2スペースは `.editorconfig`、1行の目安100文字、シングルクォート）です。プロンプトの文字列内容は自動で改行・変更しません。配布用の `dist/` ではなく、`apps/`・`packages/` 内のソースを編集してください。
