...
liveモードでは動画無効・参照画像なし・動画予算不足でも文章のみ生成します。既存のResponses全体予算を使い、1プレイのstory生成は最大1回です。動画無効時にもこの文章生成分のAPI利用が発生します。mock・中断では生成せず、画像や動画の失敗でも保存済み文章は残ります。文章のみの期限は最大120秒（ENDING_JOB_TIMEOUT_SECONDSが短ければその値）です。定義と仕様は[エンディングタグ](../specs/ending-tags/spec.md)を参照してください。
