...
## ハーネス改善（写真の自動使用・確定前の訂正）

写真の用途が明らかな場合は、送信をきっかけに現在の障害へ行動します。用途不明や具体的な危険がある場合だけ質問します。音声による中止・訂正は判定中にも受け付け、確定前の古い結果を無効化します。通常のResponses枠とは別に制御分類を1件だけ許可し、全体の並列数・試行予算は共有します。
