...
調査日：2026-09-13。公開された記法とプロンプトを確認。掲載動画の視聴による再現性評価、本プロジェクトの文字入りH3動画は生成・回収済み。内容の品質評価は人のレビュー待ち。
...
- 開発用`staging.py`では`ending_title`と`title_at`を指定できる。過去の出力ファイルに文字禁止が残っていれば再生成する。通常プレイでは監督用文章プロンプトへ直接統合する。
