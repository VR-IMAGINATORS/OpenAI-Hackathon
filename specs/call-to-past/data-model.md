...
H3 resultのreceiptはversion/endpoint/request_id/manifest_sha256/start_image_sha256/end_image_sha256/video（file/sha256/bytes）/completed_atを持つ。送信時のsubmission-attempt.approval_sha256と現approval.json、saved request ID、success記録、承認素材を照合。live attach-videoはh3-runとreceiptを要求し、動画本体と承認素材を検証する。標準はmanifest.end_imageとreceipt.end_image_sha256にタイトル入り終了画像の記録とハッシュを持ち、登録した終了画像と照合する。承認済み旧1画像runは両フィールドの明示的nullと終了画像なし登録を維持する。ending-packet.productionはinput_mode=start_and_end_frames、generate_end_frame=true、generate_storyboard=false、title_mode=end_frame_embedded、ending_title（happyはSUCCESS!!、normal/badはto be continued...）、title_position（happyはlower_center、normal/badはlower_right）、title_transition=brief_amber_light_sweep_then_stable_hold、continuity_reference（最後のゲーム画像、ない場合null）を返す。

新規生成は結末の場面と指定文字を終了画像に残し、暗転や旧ラベルを併記しない。保存済みの種別happy/normal/badや過去の承認済み素材・receiptは変更しない。
