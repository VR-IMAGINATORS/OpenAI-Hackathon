...
- 終了callbackで完成・検査済みの場面画像を固定する。最終画像を待たず、終了時のgameVersion以下で最新の完成画像を人物・道具・舞台の参照に使う。同じversionなら後の場面を優先し、完成順では選ばない。全確定行動の行動前画像も同時点で固定し、同じgameVersionの参照を重複させない。完成画像がゼロの場合だけ、文章生成と並行して終了から最大30秒の待機を開始し、進行中の画像が完成すれば余韻の素材として使う。全画像失敗・キャンセル・期限・結果失効で終了し、新規画像生成は始めない。古い画像のversionと目標factsを脚本・画像編集・検査へ明示し、確定した変化を反映する。既存12秒の文字起こし猶予は維持する。
- 開始画像と終了画像は順に生成する。各フレーム最大2回で通信障害・不正画像・検査不合格から回復する。検査の一時障害・不明・不正応答では、同じ生成済み画像を最大2回検査してから残り生成枠で修復する。検査は最大8回。重大な状態矛盾・文字・両端の連続性を検査し、終了画像検査には開始画像も使う。途中場面の検査も同じ画像での再検査を優先し、最大2生成・4検査。全体予算・期限・並列制限は維持し、認証失敗・明示的拒否・キャンセルを再試行しない。
...
- 映像候補を全確定行動へ広げる。全履歴とinventoryからアイテム一覧を作り、全アイテムを含む案→見せ場を共有する圧縮案→余韻を比較する。画像不足時は確定した結果・反応だけを描く。
  - ゲーム中の画像生成・検査には直前の行動の用途・確定結果・行動後のfactsと道具状態を渡す。画像用に行動前のfactsと道具状態を並べず、使用前後の比較を誘発しない。ending-image-service.tsには各端点の選択行動と、その時点の道具状態を渡す。終了時点の一覧から消えた消耗品も履歴から取り戻す。元の全行動履歴は動画用に保持する。
  - 静止画の構図はstill-image-composition.tsに共通化する。ゲーム中の通常・調査型の生成、両端画像の生成、両画像検査へ適用し、composition:single_momentを場面画像の既知の検査規則に加える。調査型の生成には公開情報とこの共通規則だけを渡し、非公開の検査文を戻さない。演出AIではこの制約をstartPrompt/endPromptに限定し、videoPromptの複数カットは維持する。画像検査の不合格は既存の修復・再生成処理を使う。[構図の修正と検証](single-scene-verification.md)。
  - 2026-09-15全アイテムの補足: ending-ai.tsのusedActionIdsは全確定行動を許し、preferredSequenceActionIdsは最も早い完成済み行動前画像からの時系列候補を提示する。成功順・直近順で絞らない。itemCoverageを同じ演出要求の出力に含め、欠落・重複・未知ID・架空の組み合わせ・時系列／ショットの矛盾を検証する。長いIDを短い別名にして照合し、要求が大きい場合は重複factsを差分にする。それでも大きければ、用途／結果の文章を対で省略して明示し、原文を含まない行動の再演を禁止する。異なる道具の初回使用を優先して文章を残す。既存の要求上限、画像待機、演出1回・2画像・15秒・768P・balanced・4,096出力tokensを維持する。詳細と検証は[item-coverage-verification.md](item-coverage-verification.md)。
...
- submit開始前にattemptを消費して固定し、`X-Fal-No-Retry: 1`を付ける。POST結果が曖昧でもrequest IDがあれば、その要求のstatus / resultの回収を続ける。IDなしなら再送せず未確認枠を維持する。結果GET・動画ダウンロード・MP4検証失敗は最大3回まで取得を再試行する。不正URL・リダイレクト・過大応答は再試行しない。
...
| 発見した懸念 | 計画への反映 |
| --- | --- |
| 15秒に多数の道具を含めると動作が短くなる | 全アイテムから設計し、移動・反復を先に圧縮。省略理由を対応表へ残し、実映像の認識率と生成時間を比較 |
