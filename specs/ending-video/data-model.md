...
`CommittedEndingAction`は行動ID、順序、ギミックID、用途、道具名と状態、前後のfacts、成功、narrative、解除の有無を保存する。全行動結果を使って結末を考え、全行動に前後の画像参照を付けて映像化の候補にする。保存数はシナリオの最大行動数以下。
...
- 全行動の前後facts、actionScenesの参照画像とそのgameVersion。画像がなければ行動前の姿を推測しない。
...
物語材料の選別は本文を上限付きのチャンクに分けて行い、出典ID付きの伏線を抽出する。最大6回の抽出、story通常1回（出力検証の修正込み最大2回）、動画有効時のみdirection最大1回。映像側は抽出済みの証拠を再利用する。抽出内容は照合可能な原文参照を必須とし、存在しないIDを拒否する。候補比較はdirectionの構造化出力に含める。種別・解除数・成否はサーバーで固定し、生成結果で上書きしない。映像脚本生成失敗でも公開済みのタグと短文を維持する。

演出AI失敗時は、`EndingDesign`と同じ形式でローカルの余韻演出を構成する。mode=aftermath、usedActionIds=[]、usedEvidenceIds=[]とし、itemCoverageには各道具の専用場面を保証できないことをomittedと理由で記録する。公開済み文章と確定状態を変更しない。画像は各フレーム最大2生成、各生成画像最大2検査（合計最大8検査）。終了時に完成参照画像がゼロなら、Jobに最大30秒の参照待機Promiseを持ち、文章生成と並行して既存場面の完成を待つ。待機はジョブ終了時に中止し、全体期限を延長しない。
