...
文章用AIは`title / story / evaluation`、主役tag（固定ID・根拠行動ID・理由、またはnull）、伏線sourceIdを生成する。storyは最大240文字。検証後すぐ保存し、動画の処理枠・脚本生成を待たず公開する。映像用AIはその公開済み文章と証拠を入力にし、伏線sourceId・行動ID、候補比較、画像・動画promptだけを生成する。内部の`EndingDesign`は両者を組み合わせた型だが、映像応答で文章・タグを上書きしない。表示文は選択言語、画像・動画promptは英語を使う。
