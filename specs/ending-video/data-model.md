...
演出AI失敗時は、`EndingDesign`と同じ形式でローカルの余韻演出を構成する。mode=aftermath、usedActionIds=[]、usedEvidenceIds=[]とし、公開済み文章と確定状態を変更しない。画像は各フレーム最大2生成、各生成画像最大2検査（合計最大8検査）。終了時に完成参照画像がゼロなら、Jobに最大30秒の参照待機Promiseを持ち、文章生成と並行して既存場面の完成を待つ。待機はジョブ終了時に中止し、全体期限を延長しない。
