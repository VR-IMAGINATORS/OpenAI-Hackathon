...
- 変更 hosted-runtime.tsのtick: warningSent一個をschedulerへ置換。先にgame.check、終了後は破棄。2026-09-15の統合で、通常警告はquietを確認してもthinking.appendの保留情報とし、開始時の指示でLiveに自然な切れ目を選ばせる。最終警告は待機上限とcommentaryの切替表現を維持。毎回のinstructions.appendは送らない。通常警告は結果へ結合せず、結果の後に内部情報として渡す。最終警告だけは結果と一つの発話へ結合できる。
