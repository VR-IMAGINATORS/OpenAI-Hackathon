...
- H3: hosted-runtime.tsでH1/H2を統合。結果と次の状況を別通知。残り時間はcore-config設定で60秒未満の一度通知。instructions.appendを通知契約に追加。2026-09-15の調整で時間通知はthinking.appendの保留情報へ変更し、会話の切れ目で添える方針をセッション開始時に指定する（委譲回復のinstructions.appendは維持）。終了・再接続・判定中に古い通知/結果を復活させない。
