...
追加の境界検証（T14〜T17）は [ending-second-pass.md](ending-second-pass.md) に記録。全行動の構造化情報を保った長文入力の縮小、複数の不正を含む応答の修正順序、外側JSONと拒否の区別、結末用メモリ予約・期限管理、抽出による本文予算消費の防止を実装し、120組み合わせを含む134ケースを追加する。

2026-09-15の再発対応（T12,T13）: 抽出を15秒に制限し、不正出力や抽出用入力の上限超過から元の提示記録で文章生成へ回復する。通常の抽出結果も合計48 KiB、回復時は元記録32 KiB以内。入力全体が112 KiBを超える場合は行動内の重複状態を変化だけに圧縮し、証拠量を残り容量に合わせる。タグ候補を事前に検証し、タグ検証失敗後の修正はnullだけを許す。抽出失敗と最終文章失敗のログを分け、引用不一致件数と入力の件数・バイト数を記録する。開始待機・キャンセルの失敗もstoryErrorCodeへ保存する。
